...
def GCP_detect(video_path: str) -> list:
    ...
    GCP_ref = cv2.threshold(cv2.imread(path.join(path.dirname(__file__),"gcp_reference.png"), cv2.IMREAD_GRAYSCALE), 128, 255, cv2.THRESH_BINARY)[1]
...
def sort_src(src):
    ...
    if len(second_quad) == 1:
        ...
        if np.abs(second_quad[0][0]-second_quad[1][0]) < 0.25 * np.abs( 0.5*(second_quad[0][0]+second_quad[1][0]) - x_cent):
