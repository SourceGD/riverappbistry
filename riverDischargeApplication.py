...
class CreateBathPopup(Popup):
    ...
    def RectTextInput(self):
        ...
        self.nameFile = TextInput(multiline=False, hint_text='name of the file', size_hint=(0.45, 0.2),
                                  pos_hint={'top': 1.65, 'center_x': 0.5})
        self.L = TextInput(multiline=False, hint_text='L value', size_hint=(0.12, 0.2),
                           pos_hint={'top': 1.2, 'center_x': 0.5})
        self.H = TextInput(multiline=False, hint_text='H value', size_hint=(0.13, 0.2),
                           pos_hint={'center_y': 0.5, 'right': 0.91})
    ...
    def TrapTextInput(self):
        ...
        self.L = TextInput(multiline=False, hint_text='L value', size_hint=(0.12, 0.2),
                           pos_hint={'top': 1.2, 'center_x': 0.485})
        self.H = TextInput(multiline=False, hint_text='H value', size_hint=(0.13, 0.2),
                           pos_hint={'center_y': 0.5, 'right': 0.91})
        self.B = TextInput(multiline=False, hint_text='B value', size_hint=(0.13, 0.2),
                           pos_hint={'center_x': 0.485, 'top': 0})
        self.RefPoint = TextInput(multiline=False, hint_text='Coordinates', size_hint=(0.19, 0.2),
                                  pos_hint={'right': 0.3})
        self.ids.RelLayout.add_widget(self.nameFile)
        self.ids.RelLayout.add_widget(self.L)
    ...
    def FreeShapeTextInput(self):
        ...
        self.ids.GridLayoutBath.spacing = '60sp'
        self.nameFile = TextInput(multiline=False, hint_text='name of the file', size_hint=(0.5, 0.2),
                                  pos_hint={'top': 1.85, 'center_x': 0.5})
        self.X = TextInput(multiline=False, hint_text='X value', size_hint=(0.5, 0.2),
                           pos_hint={'top': 1.6, 'center_x': 0.5})
        self.Y = TextInput(multiline=False, hint_text='Y value', size_hint=(0.5, 0.2),
                           pos_hint={'top': 1.4, 'center_x': 0.5})
        self.ids.RelLayout.add_widget(self.nameFile)
...
class OrthoRectification(Screen):
    ...
    def LoadPts(self):
        ...
        if (self.pts_src == []):  # if the user did not select any points and try to load
            ...
            pts_src, pts_mask, img, L = app.ORTHOPARAMS['pts_src'], app.ORTHOPARAMS['pts_mask'], app.ORTHOPARAMS['img'], \
            app.ORTHOPARAMS['L']
...
class DEMPopup(Popup):
    ...
    def showBathPlot(self, f):
        ...
        precision = float(
            self.ids.PrecisionID.text) * 1e-3  # precision given in mm/pixel but in the following it must be in meter
...
class ROIMASK(Screen):
    ...
    def onclick(self, event):
        if self.choiceROI:
            ...
            x, y = event.inaxes.transData.inverted().transform((event.x, event.y))
            self.pts_ROI += [x, y]
            # when ROI points selected, we change the points so the order is correct
            if len(self.pts_ROI) == 4:
                x_max = max(self.pts_ROI[0], self.pts_ROI[2])
                x_min = min(self.pts_ROI[0], self.pts_ROI[2])
                y_max = max(self.pts_ROI[1], self.pts_ROI[3])
                y_min = min(self.pts_ROI[1], self.pts_ROI[3])
                self.pts_ROI = [x_min, y_min, x_max, y_max]
            self.fig.canvas.draw()
    ...
    def registerInformation(self):
        ...
        if self.choiceROI:
            if (
                    self.pts_ROI == []):  # the user can now register and load without select ROI and the program doesn't close anymore
...
class Filters(Screen):
    ...
    def plotHistogram(self, filepath):
        ...
        app = App.get_running_app()
        filepathHisto = applyFilterOnVideoFrames(filepath, app.VIDEORANGE, self.filtersChosen,
                                                 self.parametersFilterChosen)
    ...
    def saveFilterChosen(self):
        ...
        if len(app.FILTERS) != 0:
            ...
            app.VIDEOFILEPATH = applyFilterOnVideoFrames(app.VIDEOFILEPATH, app.VIDEORANGE, app.FILTERS,
                                                         app.FILTERSPARAM)
...
class SurfaceVelocity(Screen):
    ...
    def computeVelocity(self, winsize, overlap, SNR):
        ...
        nbFramOfInterest = (app.VIDEORANGE[1] - app.VIDEORANGE[
            0]) * fps / step  # ajout du /step car barre de chargement ne tenait pas compte du step

        i = 0
        frameVideo = {}
        dt = 1.0 / (fps / step)  # sec
        print('Infos: The video has ', fps, ' frames per second and you selected a step of ', step,
              ' so the delta t is ', dt)
        velocity_field = {}
        ...
        if app.CALIB == True or app.FILTERS != []:
            ...
            ind_start = 0
            ind_end = min(int(app.VIDEORANGE[1] * fps) - int(app.VIDEORANGE[0] * fps) - step,
                          len(frameVideo) - 1 - step)
        else:
    ...
    def saveInformation(self):
        ...
        if app.ORTHO == False:
            L_pixel = np.sqrt((self.pts_section[1, 0] - self.pts_section[0, 0]) ** 2 + \
                              (self.pts_section[1, 1] - self.pts_section[
                                  0, 1]) ** 2)  # used for to calculate the scalingFactorWithoutOrtho
            ...
            print('Infos: The calculated scaling factor without orthorectification is ',
                  self.scaling_factorWithoutOrtho)
...
class RetrievSectionVelocityVectors(Screen):
    ...
    def on_enter(self):
        ...
        if app.ORTHO == True:
            ...
            self.img, self.h, self.Xmin, self.Ymin, self.scaling_factor = app.ORTHOPARAMS['img_rectified'], \
            app.ORTHOPARAMS['h'], app.ORTHOPARAMS['Xmin'], app.ORTHOPARAMS['Ymin'], app.ORTHOPARAMS['scaling_factor']
    ...
    def saveInformation(self):
        ...
        if app.ORTHO == True:
            ...
            self.x_interest, self.y_interest, self.u_interest, self.v_interest = retrieveSectionInformation(x, y,
                                                                                                            u_mean,
                                                                                                            v_mean, ROI,
                                                                                                            self.img,
                                                                                                            self.img_rotated,
            ...
            img = app.IMG
            self.x_interest, self.y_interest, self.u_interest, self.v_interest = retrieveSectionInformationWithoutOrtho(
                x, y, u_mean, v_mean, ROI, self.img,
    ...
    def showVelocityVector(self):
        ...
        self.ax.quiver(self.x_interest, self.y_interest, self.u_interest, self.v_interest, color='orange', width=0.005,
                       angles='xy')
        widget = FigureCanvasKivyAgg(self.fig)
        self.ids.SectionGraphID.add_widget(widget)
...
class DischargeComputation(Screen):
    ...
    def compute(self, coefficient):
        ...
        u_mean = app.MEANVELOCITYDATA['u_mean']
        v_mean = app.MEANVELOCITYDATA['v_mean']
        np.save('analysisResults.npy',
                {'x': x, 'y': y, 'u_mean': u_mean, 'v_mean': v_mean, 'water_level': app.WATERLEVEL,
                 'discharge': discharge})
